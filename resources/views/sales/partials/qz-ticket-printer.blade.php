...
<script>
(function () {
    if (window.__salesQzTicketPrinterLoaded) {
        return;
    }

    window.__salesQzTicketPrinterLoaded = true;
    ...
    function setButtonsState(saleId, isPrinting) {
        document.querySelectorAll('[data-ticket-sale-id="' + saleId + '"]').forEach(function (button) {
    ...
    window.printSaleTicket = async function printSaleTicket(saleId) {
        const numericSaleId = Number(saleId);
        if (!Number.isFinite(numericSaleId)) {
            alert('No se pudo identificar la venta para impresion. Recarga la pagina e intenta nuevamente.');
            return;
        }

        setButtonsState(numericSaleId, true);

        try {
            await ensureQzConnection();

            const payloadResponse = await fetch(`{{ url('/sales') }}/${numericSaleId}/ticket-payload`, {
                method: 'GET',
                credentials: 'same-origin',
                headers: {
                    'Accept': 'application/json',
            ...
            const config = qz.configs.create(printerName, {
                ...
                copies: 1,
                jobName: payload.job_name || `ticket-${numericSaleId}`,
            });
            ...
            setButtonsState(numericSaleId, false);
        }
    };
})();
</script>
